...
The Serving stage no longer holds StickSplit / the CV Drumming Analytics Tool — it was replaced (on the user's request, even though that project had real metrics) with the **Payment & Vendor Analytics Dashboard** (`id: 'vendor-analytics-dashboard'`), Chris's data-engineering-lead + assistant-project-lead role at the Cal Poly Systems Optimizations Club (Sep 2026 — Present): a Python scrape → AWS S3 data lake → OpenStreetMap/topography geocode enrichment → Great Expectations + Anthropic/OpenAI semantic-fallback validation → map-based dashboard pipeline for a South African payment/social-welfare platform. No `links` and no `architecture` diagram yet — the system is still being scoped; both are data-only adds later. That role was also added to `about.js`'s `experience` list.
