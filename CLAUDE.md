...
The Serving stage no longer holds StickSplit / the CV Drumming Analytics Tool — it was replaced (on the user's request, even though that project had real metrics) with the **Payment & Vendor Analytics Dashboard** (`id: 'vendor-analytics-dashboard'`), Chris's data-engineering-lead + assistant-project-lead role for **Ataa Pay** (a South African payment / social-welfare platform), a project obtained *through* the **Cal Poly Systems Optimizations Club** (Sep 2026 — Present): a Python scrape → AWS S3 data lake → OpenStreetMap/topography geocode enrichment → Great Expectations + Anthropic/OpenAI semantic-fallback validation → map-based dashboard pipeline. On the site the client/employer is **Ataa Pay** — the `about.js` `experience` entry reads "…— Ataa Pay", while the project card leads with the Systems Optimizations Club (the `company` byline is `Cal Poly Systems Optimizations Club · Ataa Pay, …` and the description's first line names the club first), per direct user instruction. Its description is deliberately em-dash-free (also a user request). No `links` and no `architecture` diagram yet — the system is still being scoped; both are data-only adds later.

Still open: live demo URLs for the projects (see Known gaps below). Deployment: a `render.yaml` static-site blueprint now exists at the repo root (`npm install && npm run build`, publish `./dist`, `autoDeploy: true`, `/* → /index.html` rewrite), plus `public/_redirects` carrying the same rewrite as a bundled file. Whether a Render service is actually connected to `BubbaLubz/chrisenriquezdev` and auto-deploying is a dashboard question, not a repo one — if pushes aren't deploying, the usual cause is the repo rename (`bubba-portfolio` → `chrisenriquezdev`) dropping the repo from the Render GitHub App's allowed list, or Auto-Deploy being off / pointed at the wrong branch. Contact mechanism is actually already resolved (a `ContactOverlay.jsx` modal exists and is wired to the Output "Contact Information" card) — this doc's Open Decisions section below is stale on that point specifically.
...
`about.js`, `projects.js`, and `skills.js` now hold real content, sourced from the resume dropped at the project root (also copied to `public/resume.pdf`, referenced by `about.resume`) plus a lookup against the public GitHub repos at `github.com/BubbaLubz`. Stage mapping (current — check `projects.js`'s `stageBadge` fields directly if this ever looks stale): **Ingestion** = Cloud Music Analytics Platform, **Transformation** = PFAS Analytic Pipeline, **Storage** = AI Data Infrastructure Diagram Tool, **Serving** = Payment & Vendor Analytics Dashboard (Ataa Pay, via the Cal Poly Systems Optimizations Club — replaced StickSplit). The PFAS project is the one prior real work experience (EKI Environment and Water Inc.), not a separate listed "project" — see the resume's own Experience section. This was a deliberate choice, confirmed with the user, since the resume lists only 3 "Projects" against 4 stage slots.

**Known gaps to fill in**: the Payment & Vendor Analytics Dashboard has no repo/demo links yet (still being scoped) and no `architecture` diagram — add both as data-only changes once available. None of the projects has a confirmed live demo URL (the resume lists "DEMO" as link text, but the PDF extraction doesn't expose the underlying href, and GitHub's `homepage` field was empty for the matched repos). Get the real URLs from the user before treating `projects.js`'s `links` fields as final.

## Open Decisions (deferred, revisit before or during build)

- **Deployment target**: Render (Static Site). `render.yaml` blueprint is now committed at the repo root — `npm install && npm run build`, publish `./dist`, `autoDeploy: true`, and a `/*` → `/index.html` **rewrite** (not redirect) since React Router's client-side routes 404 on a hard refresh without it; `public/_redirects` carries the same rule for a manually-created service. Still unconfirmed whether a Render service is connected to the repo and auto-deploying — that's dashboard config: create a service (New → Blueprint reads `render.yaml`, or New → Static Site manual), then Settings → Build & Deploy → Auto-Deploy = Yes on branch `main`. If it was connected before the repo rename and stopped deploying, re-authorize the repo in GitHub → Settings → Applications → Render → Configure.
